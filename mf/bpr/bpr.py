...
class BPR(object):
    ...
    def __init__(self, rank, n_users, n_items, lambda_u = 0.0025, lambda_i = 0.0025, lambda_j = 0.00025, lambda_bias = 0.0, learning_rate = 0.05):
        ...
        self._generate_pred_model_function()
        self._generate_batch_pred_model_function()
    ...
    def _generate_batch_pred_model_function(self):
        
        u = T.lvector('u')
        i = T.lvector('i')
        
        pred2A =  T.diag( T.dot(self.W[u], self.H[i].T) )
        pred2 = T.add( pred2A, self.B[i] )
        self.batch_prediction = theano.function(inputs=[u,i], outputs=pred2)
    ...
    def train(self, train_data, epochs=30, batch_size=1000):
        ...
        self._train_dict, self._train_users, self._train_items = self._data_to_dict_pandas( train_data )
...
#         res = user_vector.dot(h.T) + b
#         return res
        return self.get_user_res(user_index)
    
    def user_vector(self, user_index):
        """
          Computes item predictions for `user_index`.
          Returns an array of prediction values for each item
          in the dataset.
    ...
    def item_vector(self, item_index):
        """
          Computes item predictions for `user_index`.
          Returns an array of prediction values for each item
          in the dataset.
    ...
    def prediction_batch(self, user_index, item_index):
        """
          Predicts the preference of a given `user_index`
          for a gven `item_index`.
        """
        return self.batch_prediction(user_index, item_index)
    
    def predictionT(self, user_index, item_index):
        """
          Predicts the preference of a given `user_index`
          for a gven `item_index`.
        """
        return self.get_user_item_res( user_index, item_index )
    ...
    def _data_to_dict_pandas(self, data):
